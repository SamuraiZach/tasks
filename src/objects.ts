...
export function isCorrect(question: Question, answer: string): boolean {
    const trimmed = question.expected.trim();
    const lowercased = question.expected.toLowerCase();
    const Atrimm = answer.trim();
    if (question.expected === answer) {
        return true;
    } else if (trimmed === Atrimm) {
        return true;
    } else if (lowercased === answer) {
        return true;
    } else {
        return false;
    }
...
export function isValid(question: Question, answer: string): boolean {
    if (question.type === "multiple_choice_question") {
        const optionsX = [...question.options];
        const findAnswer = optionsX.some((q: string): boolean => q === answer);
        if (findAnswer) {
            return true;
        } else {
            return false;
...
export function toShortForm(question: Question): string {
    const lenx = question.name.length;
    if (lenx > 10) {
        const str = question.name.substr(0, 10);
...
export function toMarkdown(question: Question): string {
    if (question.type === "multiple_choice_question") {
        const optionsX = [...question.options];
        const mapChoices = optionsX.map((name: string): string => "- " + name);
        const fullChoices = mapChoices.join("\n");
